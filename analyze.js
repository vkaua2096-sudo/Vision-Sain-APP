// API serverless do Vercel - chama Claude Vision com segurança
// A API Key fica no servidor, não exposta no navegador
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // API Key vem das variáveis de ambiente do Vercel (segura!)
...
{"erro": "Imagem não permite análise. Por favor, envie uma foto bem iluminada do rosto."}`;
    ...
    if (!response.ok) {
      ...
      console.error('Erro da Anthropic:', errorData);
    ...
    // Se a IA retornou erro, repassar
    if (diagnosis.erro) {
